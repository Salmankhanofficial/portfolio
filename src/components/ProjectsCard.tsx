...
const ProjectsCard = ({ icon, title, description, onClick,link }: Props) => {
  return (
    <div
      ...
      onClick={onClick} 
    >
      <div className="w-full flex justify-center items-center h-full overflow-hidden top-0 opacity-0 bg-gray-100 absolute z-50 group-hover:opacity-70">
        <h1 className="text-2xl text-[#FF6300] opacity-95 font-bold">Preview</h1>
      </div>
      <div className="w-11/12 p-2 rounded-xl transform transition-transform duration-300 group-hover:scale-110 h-full">
        <Image src={icon} alt={"image"} className="w-full h-96" />
